...
function renderGoals(){const e=el('goals-list');if(!e)return;if(!S.goals.length){e.innerHTML='<div style="color:var(--muted);font-size:0.68rem">No goals set.</div>';return;}e.innerHTML=S.goals.map((g,i)=>`<div class="goal-item ${g.done?'goal-done':''}" draggable="true" ondragstart="goalDragStart(${i})" ondragover="goalDragOver(event,${i})" ondrop="goalDrop(event,${i})"><span style="color:var(--muted);cursor:grab;flex-shrink:0">☰</span><div class="chk-box ${g.done?'done':''}" onclick="event.stopPropagation();toggleGoal(${i})"></div><div class="goal-text">${g.text}</div><button class="btn xs rd" onclick="event.stopPropagation();delGoal(${i})">✕</button></div>`).join('');renderGoalsDash();}
function renderGoalsDash(){const e=el('goals-dash-list');const se=el('goals-dash-status');if(!e)return;if(!S.goals.length){e.innerHTML='<div style="color:var(--muted);font-size:0.68rem">No goals set.</div>';if(se)se.textContent='';return;}const done=S.goals.filter(g=>g.done).length;if(se)se.textContent=`${done}/${S.goals.length} Complete`;e.innerHTML=S.goals.map((g,i)=>`<div style="display:flex;align-items:flex-start;gap:8px;padding:6px 0;border-bottom:1px solid var(--border)"><div class="chk-box ${g.done?'done':''}" onclick="toggleGoal(${i})" style="width:16px;height:16px"></div><div style="font-family:var(--mono);font-size:0.78rem;color:${g.done?'var(--muted)':'var(--text)'};${g.done?'text-decoration:line-through':''};">${g.text}</div></div>`).join('');}
